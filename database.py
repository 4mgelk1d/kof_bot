...
class Database:
    ...
    def init_db(self):
        with self.get_conn() as conn:
            ...
            # Таблица скопированных постов (чтобы не дублировать)
            cursor.execute('''
    ...
    def get_profiles(self, user_id: int) -> List[Dict]:
        with self.get_conn() as conn:
            ...
            for row in cursor.fetchall():
                p = dict(row)
                p['source_channels'] = json.loads(p['source_channels'])
                p['target_channels'] = json.loads(p['target_channels'])
                profiles.append(p)
            return profiles
    ...
    def update_profile_sources(self, profile_id: int, sources: List[int]):
        with self.get_conn() as conn:
            cursor = conn.cursor()
            cursor.execute("UPDATE profiles SET source_channels = ? WHERE id = ?", (json.dumps(sources), profile_id))
    
    def update_profile_targets(self, profile_id: int, targets: List[int]):
        with self.get_conn() as conn:
            cursor = conn.cursor()
            cursor.execute("UPDATE profiles SET target_channels = ? WHERE id = ?", (json.dumps(targets), profile_id))
    
    def update_profile_schedule(self, profile_id: int, schedule_date: str):
        with self.get_conn() as conn:
            cursor = conn.cursor()
            cursor.execute("UPDATE profiles SET schedule_date = ? WHERE id = ?", (schedule_date, profile_id))
    
    def update_profile_active(self, profile_id: int, is_active: int):
        with self.get_conn() as conn:
            cursor = conn.cursor()
